...
class PriceConverter {
  static String convertPrice(double? price,
    ...
    return '${isRightSide ? '' : Get.find<SplashController>().configModel!.currencySymbol} '
        '${(toFixed(price!, Get.find<SplashController>().configModel!.digitAfterDecimalPoint!)).toStringAsFixed(asFixed ?? Get.find<SplashController>().configModel!.digitAfterDecimalPoint!).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'
  ...
  static double toFixed(double val, int places) {
