...
class PasswordCheckWidget extends StatelessWidget {
  ...
  const PasswordCheckWidget(
      {super.key, required this.title, required this.done});
    ...
    return Padding(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        Icon(done ? Icons.check : Icons.clear,
            color: done ? Colors.green : Theme.of(context).colorScheme.error,
            size: 12),
        Text(title,
            style: robotoRegular.copyWith(
                color:
                    done ? Colors.green : Theme.of(context).colorScheme.error,
                fontSize: 12)),
      ]),
    );
  }
