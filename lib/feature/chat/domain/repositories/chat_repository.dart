...
class ChatRepository implements ChatRepositoryInterface {
  ...
  Future<ConversationsModel?> searchConversationList(String name) async {
    ...
    Response response = await apiClient.getData(
        '${AppConstants.searchConversationListUri}?name=$name&token=${_getUserToken()}&limit=20&offset=1');
    if (response.statusCode == 200) {
  ...
  Future<Response> getMessages(
    ...
    return await apiClient.getData(
        '${AppConstants.getMessageListUri}?${conversationID != null ? 'conversation_id' : userType == UserType.user ? 'user_id' : 'vendor_id'}=${conversationID ?? userId}&token=${_getUserToken()}&offset=$offset&limit=10');
  }

  @override
  Future<Response> sendMessage(String message, List<MultipartBody> file,
