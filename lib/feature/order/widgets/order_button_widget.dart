...
class OrderButtonWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    int selectedIndex = fromMyOrder
        ? (orderController.selectedMyOrderStatusIndex ?? 0)
        : (orderController.selectedRunningOrderStatusIndex ?? 0);
    bool isSelected = selectedIndex == index;

    return InkWell(
      onTap: () {
        if (fromMyOrder) {
          orderController.setSelectedMyOrderStatusIndex(
              index, statusListModel.status);
          orderController.getCompletedOrders(
              offset: 1, status: statusListModel.status);
        } else {
          orderController.setSelectedRunningOrderStatusIndex(
              index, statusListModel.status);
          ...
          child: Row(
            children: [
              ...
              Container(
                ...
                child: Text(
                  fromMyOrder
                      ? orderController.completedOrderCountList![index]
                          .toString()
                      : orderController.currentOrderCountList![index]
                          .toString(),
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
